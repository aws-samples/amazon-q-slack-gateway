...
**Note:** The original instructions were for Okta, but this is a specific implementation using AWS Cognito.
...
3. You need to have users subscribed to your Amazon Q business application, and are able to access Amazon Q Web Experience. If you haven't set one up yet, see [Subscribing users to an Amazon Q application](https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/adding-users-groups.html)

4. You have aws cli latest version installed on your Linux or MacOS system. If you haven't installed yet, see [Installing the AWS CLI version 2](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html)


### 1. Setup

#### 1.1 Create an OIDC app integration, for the gateway, in AWS Cognito.

1. Create the Cognito User Pool with name: "AWSAmazonQ", select `Application type: Traditional web application`

2. Go to the Newly Created pool and Select `Set up your app: AWSAmazonQ`

3. In the `Login pages:` tab: 
* Add as a  `Allowed callback URLs` the `Invoke URL` from the AWS API Gateway that contains the following keyworkds: `OIDCCallbackApi` (That can be found In the `Stages` option from the left menu)
* Add as a `Identity providers` : `Cognito user pool`
...
Create trusted token issuer to trust tokens your AWS Cognito tenant using these instructions listed here - https://docs.aws.amazon.com/singlesignon/latest/userguide/using-apps-with-trusted-token-issuer.html.
Or you can run the below script.

For the script, you need to have the OIDC issuer URL and the AWS region in which you have your Q business application. To retrieve the OIDC issuer URL, go to the AWS Cognito console, click the left hamburger menu and open Security > API and copy the whole 'Issuer URI'. The IAM IdC region is typically the same region where your Amazon Q Business application has been created but that is not a requirement.
...
```
 ...
 OIDC_ISSUER_URL="https://cognito-idp.us-east-1.amazonaws.com/us-east-1_xxxxxx"
 AWS_IDC_REGION="us-east-1"
 bin/create-trusted-token-issuer.sh $OIDC_ISSUER_URL $AWS_IDC_REGION


TTI_ARN=arn:aws:sso::015783273524:trustedTokenIssuer/ssoins-7223df7a3afd0770/tti-d4e8b488-2010-70dd-fdc9-212212112
```
...
```
 export AWS_DEFAULT_REGION="us-east-1"
 OIDC_CLIENT_ID="h8k4r6cg056f4111111111"
 TTI_ARN="arn:aws:sso::015783273524:trustedTokenIssuer/ssoins-7223df7a3afd0770/tti-d4e8b488-2010-70dd-fdc9-4baf93bc212212112932a"
 AWS_IDC_REGION="us-east-1"
 bin/create-idc-application.sh $OIDC_CLIENT_ID $TTI_ARN $AWS_IDC_REGION

Checking if the AmazonQSlackGateway exists...
Creating AmazonQSlackGateway...
Created GATEWAY_IDC_ARN: arn:aws:sso::015783273524:application/ssoins-7223df7a3afd0770/apl-1111111
AmazonQSlackGateway is setup with GATEWAY_IDC_ARN: arn:aws:sso::015783273524:application/ssoins-7223df7a3afd0770/apl-1111111
...
3. Enter the following parameters:
    ...
    4. `OIDCIdPName`: The name of the OIDC external identity provider. Specify `Cognito` is also supported.
    5. `OIDCClientId`: The client ID of OIDC client you created in step 1.1. That's from your Cognito App `AWSAmazonQ`, `the Client ID`
    6. `OIDCIssuerURL`: That's the `Token signing key URL` from Cognito's User Pool.
...
Go the app client settings created in AWS Cognito (in step 1.1), and update the client redirect URL with exported value in CloudFormation stack for `OIDCCallbackEndpointExportedName`.

Example:

```
Key: AQSGOIDCCallbackEndpointExportedName
Value: https://bvok8b6441.execute-api.us-east-1.amazonaws.com/prod/
Description: OIDC Callback API endpoint
Export name: AQSG-OIDCCallbackEndpointExportedName
```
...
3. Choose `Retrieve secret value`
4. Choose `Edit`
5. Replace the value of `OidcClientSecret`, you will find the value in the Cognito app client settings (step 1.1). That's the `Client secret` from `App client: AWSAmazonQ`
...
5. You'll be prompted to Sign In with your Cognito credentials to authenticate with Amazon Q. Click the button to sign in.
![Slack Sign In](./images/sign-in-demo.png)
6. You'll be redirected to browser to sign in with AWS Cognito. Once you sign in, you can close the browser window and return to Slack.
